...
print(a)
for x in np.nditer(a, order='F'): # 每一column 每一column走
    print(x)
...
for x in np.nditer(a, order='F'):  # loop through each number at the same position but in different page
    print(x)
...
for x in np.nditer(a, op_flags = ['readwrite']): #This works because we are setting the readwrite flag to readwrite mode during this loop.without that, we can not write
...
print(a)
print('The minimum element going down each column is ', np.amin(a, 0))
...
#plt.plot(x,y)
plt.plot(x,y,'bo',linestyle='dotted') #solid #dotted #dashed #dashdot 'bo' blue circle
# okiyt(x,y,color = 'Red', linestyle = 'dashed')
