...
import locale
print(help(locale))
locale.setlocale(locale.LC_ALL,'se-SE')
...
process_customers2(city='Richmond', cust_potential='high', comp_score=8)
...
def get_random_ints2 (count, begin, end):
    ...
    print('Notice when this statement gets executed! Why only once instead of ten times?') #outside for loop
...
file_name = '/Users/jing/Desktop/randomfilename.txt'
handle = open(file_name, 'w') # can rewrite
handle.write('CustID|CustName|Gender')
handle.write('\n') # seperate line
handle.write('1000|Bill Jones|Male')
handle.close()
...
file_name = '/Users/jing/Desktop/randomfilename.txt'
handle = open(file_name, 'x') #cannot exist because already exist
handle.write('CustID|CustName|Gender')
handle.write('1000|Bill Jones|Male')
handle.close()
...
import csv
file_name = '/Users/jing/Desktop/randomfilename.txt'
handle = open(file=file_name, mode='w',newline='') #keyword arguments ? original no new line
