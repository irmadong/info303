...
# half closed book -> turn in
# open book another half
...
# return f'Hello, {name.title()}. Today is a great day to learn some Python!!'

hellomsg('grant williamson')
# hellomsg()
# x = hellomsg('grant williamson')
# print(x)
...
i = 207
my_custfunc()  # i = 23 is inside the function so it inside the scope.
print(i)
...
machine_chosen = select_color()

p1 = input("Select the color:")
p2 = input("Select the color:")
p3 = input("Select the color:")
p4 = input("Select the color:")
picked = [p1, p2, p3, p4]
...
def compared(alist):
    count = 0
    for i in range(0, 4):
        if picked[i] == machine_chosen[i]:
            count = count + 1
            print(count)

    same_color = 4 - Diff(alist, machine_chosen)
    print("Correct color in the correct place: " + str(count))
    print("Correct color but the wrong place:" + str(same_color - count))
    return count


num = compared(picked)
print(machine_chosen)
print(picked)
# After the computer makes
# its four color selections, the user should enter
