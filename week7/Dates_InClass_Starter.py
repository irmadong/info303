...
tmstr = parse(date_time_str3) #error because of not standard format!!
...
from dateutil.relativedelta import * # cannot just import the dateutil
...
# need the timing for that !! to analyze the time from now \
...
ndt = today+relativedelta(day=31, weekday=WE(-1))
print(ndt)
...
#find the sum, average, and standard deviation for all appointments on Sundays.
...
file_name='E:\\Teaching\\Analytics_Courses\\Spring_2020\\INFO_303\\MattsonMaterials\\Week_7\\Appointments.txt'
handle = open(file_name, 'r')
data = csv.reader(handle, delimiter='|')
my_list = []
for row in data:
    #Let's not load the header row
    my_tuple = tuple(row)
    if my_tuple[0] != 'Patient_Id':
        my_list.append(tuple(row))
handle.close()

def list_generator(my_list, dow):
    my_list_values = []
    for row in my_list:
        parsed_date = parse(row[2])
        if parsed_date.strftime('%A') == dow:
            my_list_values.append(float(int(row[3])+int(row[4])+int(row[5])))
    return my_list_values

dow = 'Sunday'
myl = list_generator(my_list,dow)

print(f'The sum of sales on {dow} is {"${:,.2f}".format(sum(myl))}.')
print(f'The average of sales on {dow} is '
      f'{"${:,.2f}".format(statistics.mean(myl))} with a standard '
      f'deviation of {"${:,.2f}".format(statistics.stdev(myl))}.')