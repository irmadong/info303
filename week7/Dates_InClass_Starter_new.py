...
for r in data:
    row = tuple(r)
    if(row[0]!='Patient_Id'):
        my_list.append(tuple(r))
handle.close()
money = []

for row in my_list:
    timedata = parse(row[2]) #
    if(timedata.strftime('%A')=='Sunday'):
        fee = int(row[3])+int(row[4])+int(row[5])
        money.append(fee)

print(round(statistics.mean(money),2))
print(round(statistics.stdev(money),2))
